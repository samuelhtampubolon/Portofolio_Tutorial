...
export function rotateEntity(e, cx, cy, deg) {
  ...
  if (e.type === 'rect') {
    // a rotated rectangle is no longer axis aligned — promote it to a polyline
    const [x1, y1] = e.a, [x2, y2] = e.b;
    e.type = 'polyline';
    e.pts = [[x1, y1], [x2, y1], [x2, y2], [x1, y2]];
    e.closed = true;
    delete e.a; delete e.b;
    for (const p of e.pts) rt(p);
  }
  return e;
}
...
function pointLineSignedDistance(a, b, p) {
  const dx = b[0] - a[0], dy = b[1] - a[1];
  const L = Math.hypot(dx, dy) || 1;
...
function offsetDistanceFor(e, p) {
